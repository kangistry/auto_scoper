...
# =============================================================================
DEFAULT_API_KEY = "app-fd0HLVIwGDiY8QYvGdsbekQe"
...
def run_dify_workflow(qp_file_id, ms_file_id, session_name, scraper, api_key, base_url, cookies, max_retries=2):
    ...
    inputs = {
        "Years_of_papers": session_name,
        "list_of_exam_papers": [{
            "type": "document",
            "transfer_method": "local_file",
            "upload_file_id": qp_file_id
        }],
        "list_of_mark_schemes": [{
            "type": "document",
            "transfer_method": "local_file",
                    ...
                    if line_text.startswith('data: '):
                        try:
                            ...
                            if event == 'workflow_finished':
                                ...
                                print(f"  [{session_name}] Node completed: {node_title}")
...
def convert_results_to_csv(results):
    ...
    for result_data in results:
        ...
        try:
            ...
            data = parse_if_string(data)
            
            outputs_container = data.get('outputs', {}) if isinstance(data, dict) else {}
            outputs_container = parse_if_string(outputs_container)
            
            outputs = outputs_container.get('output', []) if isinstance(outputs_container, dict) else []
            outputs = parse_if_string(outputs)
            
            if not isinstance(outputs, list):
                outputs = [outputs] if outputs else []
            
            for output_group in outputs:
                output_group = parse_if_string(output_group)
                if not isinstance(output_group, list):
                    output_group = [output_group] if output_group else []
                    
                for output_item in output_group:
                    output_item = parse_if_string(output_item)
                    if not isinstance(output_item, dict):
                        continue
                    
                    json_data = output_item.get('JSON', {})
                    json_data = parse_if_string(json_data)
                    
                    questions = json_data.get('questions', []) if isinstance(json_data, dict) else []
                    questions = parse_if_string(questions)
...
def main():
    st.title("📚 Past Paper Processing Pipeline")
    st.markdown("Automate your exam paper processing workflow")
                ...
                if st.button("🚀 Process Papers", type="primary"):
                    ...
                    if parallel and max_workers > 1:
                        ...
                        for i, match in enumerate(valid_matches):
                            ...
                            # Use st.status for real-time spinner feedback
                            with st.status(f"🔄 Processing {match['session']}...", expanded=True) as status:
                                start_time = time.time()
                                st.write("📤 Uploading files to Dify...")
                                
                                # Show that workflow is running
                                st.write("⏳ Running workflow (typically 1-5 minutes for large papers)...")
                                st.write("💡 Tip: Watch this space - if no update after 5+ min, workflow may have timed out")
                                
                                result = process_single_paper(match, api_key, base_url, cookies)
                                
                                elapsed = time.time() - start_time
                                
                                if result['success']:
                                    status.update(label=f"✅ {match['session']} completed in {elapsed:.0f}s", state="complete")
                                else:
                                    status.update(label=f"❌ {match['session']} failed after {elapsed:.0f}s", state="error")
                            
                            results.append(result)
                            progress_bar.progress((i + 1) / len(valid_matches), text=f"Processed {i+1}/{len(valid_matches)}")
                            
                            if result['success']:
                                add_log(f"✅ {match['session']}: Success ({elapsed:.0f}s)")
                                qp_bytes_dict[result['session']] = result['qp_bytes']
                            else:
                                add_log(f"❌ {match['session']}: {result.get('error', 'Unknown')} ({elapsed:.0f}s)")
